...
#        exit()
    if not inpath and not url:
        print "please input --inpath or input --url, specify The need for automatic generation of the class interface,Must be a JSON structure"
        exit()
    if inpath and url:
        print "please input only one (--inpath or --url). script can perform only one in one time"
        exit()
...
def args():
    ...
    parser.add_argument("--inpath", help="parsed json file path",type=str)
